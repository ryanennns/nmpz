...
const output = args.output || 'mapillary-balanced.jsonl';
const panoOnly = Boolean(args['pano-only']);
...
for (let y = 0; y < tileCount; y++) {
  for (let x = 0; x < tileCount; x++) {
    const buffer = await fetchTile(zoom, x, y, token);
    if (!buffer) {
      tileStats.push({ x, y, count: 0, weight: 0 });
      continue;
    }

    const count = countTileFeatures(buffer, layerName, panoOnly);
    if (count === 0) {
      tileStats.push({ x, y, count: 0, weight: 0 });
      continue;
    }
    ...
    totalWeight += weight;
    tilesWithCoverage += 1;
    tileStats.push({ x, y, count, weight });
  }
}
...
console.log(`Found coverage in ${tilesWithCoverage} tiles. Allocating ${limit} locations...`);

const quotas = allocateQuotas(tileStats, limit, totalWeight);

console.log('Sampling locations per tile...');
...
for (const tile of quotas) {
  ...
  const buffer = await fetchTile(zoom, tile.x, tile.y, token);
  if (!buffer) {
    continue;
  }
  const samples = sampleTileFeatures(buffer, layerName, tile.x, tile.y, tile.quota, panoOnly, rng);
  if (samples.length === 0) {
    continue;
  }
...
if (totalWritten === 0) {
  console.error('No locations sampled.');
  outputStream.end();
  process.exit(1);
}

await new Promise((resolve) => outputStream.end(resolve));
console.log(`Done. Wrote ${totalWritten} rows to ${output}.`);
...
function countTileFeatures(buffer, layerName, panoOnly) {
