...
if (!token) {
    console.error(
        'Missing Mapillary access token. Use --token or set VITE_MAPILLARY_ACCESS_TOKEN.',
    );
    process.exit(1);
}
...
console.log(
    `Scanning ${totalTiles} tiles at z=${zoom} (${layerName} layer)...`,
);
...
if (tilesWithCoverage === 0) {
    console.error(
        'No coverage found at this zoom. Try a different zoom level.',
    );
    process.exit(1);
}

console.log(
    `Found coverage in ${tilesWithCoverage} tiles. Generating variants...`,
);
...
for (const variant of variants) {
    ...
    const variantTotalWeight = variantTiles.reduce(
    ...
    for (const tile of quotas) {
        ...
        const enriched = await enrichWithHeadings(
            samples,
            token,
            entitySleepMs,
        );
    ...
    if (totalWritten === 0) {
        ...
        console.log(
            `[${variant.name}] Wrote ${totalWritten} rows to ${outputPath}.`,
        );
    }
}
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
        try {
            const res = await fetch(url, {
                headers: { 'User-Agent': 'nmpz-map-generator' },
            });
            if (!res.ok) {
...
function allocateQuotas(tiles, limit, totalWeight) {
    ...
    while (remaining > 0 && candidates.length > 0) {
        ...
        if (
            remaining > 0 &&
            candidates.every((tile) => tile.quota >= tile.count)
        ) {
            break;
        }
    }
    ...
    for (let i = 0; i < ids.length; i += batchSize) {
        ...
        for (const entity of entities) {
            ...
            const heading = Number(
                entity.computed_compass_angle ?? entity.compass_angle ?? 0,
            );
            const coords =
                entity.computed_geometry?.coordinates ??
...
function computeWeight(tile, count, mode) {
    const latWeight = Math.max(
        0.01,
        Math.cos((tile.latCenter * Math.PI) / 180),
    );
    if (mode === 'urban') {
